...
class display(root):
    def mainMenu(self):
        ...
        FrameTop=Frame(FrameMainButton,bg=self.universalBackground,borderwidth=1,relief=FLAT)
        FrameTop.pack(side=TOP,fill=X)
        FrameBottom=Frame(FrameMainButton,bg=self.universalBackground,borderwidth=1,relief=FLAT)
        FrameBottom.pack(side=TOP,fill=X)
        DownloadButton=Button(FrameTop,text="Create a Level",command=createLevel,bg=self.universalBackground)
        DownloadButton.pack(side=LEFT,fill=X,padx=self.mainMenuPadX)
        DownloadButton=Button(FrameTop,text="Edit a level",command=editLevel,bg=self.universalBackground)
        DownloadButton.pack(side=LEFT,fill=X,padx=self.mainMenuPadX)
            
        DownloadButton=Button(FrameBottom,text="View a level",command=viewLevel,bg=self.universalBackground)
        DownloadButton.pack(side=LEFT,fill=X,padx=self.mainMenuPadX)
        DownloadButton=Button(FrameBottom,text="Quit",command=quit,bg=self.universalBackground)
        DownloadButton.pack(side=LEFT,fill=X,padx=self.mainMenuPadX)
